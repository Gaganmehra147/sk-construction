import { getAdminSession } from "@/lib/auth";
import { redirect } from "next/navigation";
import { prisma } from "@/lib/prisma";
import StatsEditorClient from "./StatsEditorClient";

export const dynamic = "force-dynamic";

export default async function AdminStatsPage() {
  const session = await getAdminSession();
  if (!session) {
    redirect("/admin/login");
  }

  const stats = await prisma.statItem.findMany({
    orderBy: { sortOrder: "asc" },
  });

  return (
    <div className="space-y-6">
      <div className="border-b border-white/10 pb-4">
        <span className="text-[10px] font-mono-tech uppercase tracking-[0.24em] text-[#C5A880]">
          CREDIBILITY METRICS
        </span>
        <h1 className="font-serif-heading text-2xl sm:text-3xl font-bold text-white mt-1">
          Trust & Experience Statistics
        </h1>
        <p className="text-stone-400 text-xs mt-1">
          Update actual verifiable project counts, years in practice, and coverage numbers displayed across the homepage and about page.
        </p>
      </div>

      <StatsEditorClient initialStats={stats} />
    </div>
  );
}
